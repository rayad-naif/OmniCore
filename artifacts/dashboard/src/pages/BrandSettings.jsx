...
function HexInput({ label, value, onChange, hint }) {
  ...
  function handleText(e) {
    ...
    if (!isValidHex(v)) {
      setErr('Must be a valid hex colour, e.g. #6366f1');
      return;
    }
    setErr('');
    onChange(v);
...
function LogoUploader({ brandId, currentLogoUrl, onUploaded, authFetch }) {
  ...
  async function handleFile(file) {
    ...
    if (file.size > 2 * 1024 * 1024) {
      setError('Max logo size is 2 MB.');
      return;
    }
    setError('');
    setUploading(true);
    ...
    try {
      ...
      const res = await authFetch(
        `${API_URL}/brands/${brandId}/logo-upload-url`,
        {
          method: 'POST',
  ...
  return (
    <Field
      label="Brand Logo"
      hint="PNG or SVG recommended. Max 2 MB."
      error={error}
    >
      <div className="flex items-center gap-5">
        {/* Preview */}
        <div
          className="w-20 h-20 rounded-2xl border-2 border-dashed border-slate-300 bg-slate-50
...
function CorsTagInput({ value, onChange }) {
  ...
  function addOrigin() {
    ...
    if (!v) return;
    if (!isValidOrigin(v)) {
      setErr('Must be a full URL, e.g. https://example.com');
      return;
    }
    if (value.includes(v)) {
    ...
    >
      ...
      {value.length > 0 && (
        <p className="mt-1.5 text-[11px] text-slate-400">
          {value.length} origin{value.length !== 1 ? 's' : ''} allowed
        </p>
      )}
    </Field>
  );
...
function WidgetPreview({ primaryColor, accentColor, brandName, logoUrl }) {
  ...
  return (
    <div
      className="relative w-full rounded-2xl overflow-hidden bg-slate-100 p-4"
      style={{ minHeight: 260 }}
    >
      {/* Fake browser chrome */}
      ...
      <div className="absolute bottom-7 right-7 flex flex-col items-end gap-3">
        {/* Chat bubble */}
        <div
          className="bg-white rounded-2xl rounded-br-sm shadow-xl border border-slate-100
...
export default function BrandSettings() {
  ...
  function validate() {
    ...
    if (primaryColor && !isValidHex(primaryColor))
      e.primaryColor = 'Invalid hex colour';
    if (accentColor && !isValidHex(accentColor))
      e.accentColor = 'Invalid hex colour';
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-slate-500 mt-0.5">
              Configure your brand's identity, widget appearance, and AI
              behaviour.
            </p>
          </div>
          <div className="flex items-center gap-3">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-5">
              <HexInput
                ...
                onChange={(v) => {
                  setPrimary(v);
                  setErrors((e) => ({ ...e, primaryColor: '' }));
                }}
                hint="Used for the launcher FAB and message bubbles."
              />
              {errors.primaryColor && (
                <p className="text-xs text-red-600">⚠ {errors.primaryColor}</p>
              )}
              ...
              <HexInput
                ...
                onChange={(v) => {
                  setAccent(v);
                  setErrors((e) => ({ ...e, accentColor: '' }));
                }}
                hint="Used for gradient FAB and hover states."
              />
              {errors.accentColor && (
                <p className="text-xs text-red-600">⚠ {errors.accentColor}</p>
              )}
            </div>
        ...
        >
          ...
          >
            <div className="flex items-center">
              ...
              />
              <span
                className="px-4 py-2.5 rounded-r-xl border border-l-0 border-slate-300 bg-slate-100
          ...
          {emailPrefix && !errors.emailPrefix && (
            <div className="flex items-center gap-3 px-4 py-3 rounded-xl bg-violet-50 border border-violet-200">
              ...
              <button
                onClick={() =>
                  navigator.clipboard.writeText(`${emailPrefix}${EMAIL_SUFFIX}`)
                }
        ...
        >
          ...
          {corsOrigins.length === 0 && (
            <div className="flex items-start gap-3 px-4 py-3 rounded-xl bg-amber-50 border border-amber-200 text-amber-800 text-xs">
              ...
              <p>
                No origins configured. Without CORS origins, widget requests
                will be blocked by browsers.
              </p>
            </div>
          )}
        </SectionCard>
