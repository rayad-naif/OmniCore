    ...
    <div className="flex flex-col items-center justify-center flex-1 gap-4 py-20 text-slate-400">
      ...
      <div className="text-center">
        ...
        <p className="text-xs text-slate-400">
          Create your first article or crawl a website to populate the knowledge
          base.
        </p>
      </div>
...
export default function KnowledgeBase() {
  ...
  const filtered = articles.filter((a) => {
    if (search) {
      ...
      if (
        ...
        !a.plain_text_content?.toLowerCase().includes(q)
      )
        return false;
    }
  ...
  const handleDelete = useCallback(
    async (id) => {
      if (
        !confirm('Delete this article? This also removes all its embeddings.')
      )
        return;
      setDeleting(id);
      try {
        await authFetch(`${API_URL}/knowledge-articles/${id}`, {
          method: 'DELETE',
        });
