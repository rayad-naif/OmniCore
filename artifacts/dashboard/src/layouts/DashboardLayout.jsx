...
function StatusDot({ status = 'online' }) {
  ...
  return (
    <span
      className={`inline-block w-2.5 h-2.5 rounded-full ring-2 ring-white ${colors[status] || colors.offline}`}
    />
  );
}
...
function Avatar({ name = '', src, size = 'md' }) {
  ...
  return (
    <div
      className={`${sz} rounded-full bg-violet-600 text-white font-semibold flex items-center justify-center select-none`}
    >
      {initials || '?'}
    </div>
  );
