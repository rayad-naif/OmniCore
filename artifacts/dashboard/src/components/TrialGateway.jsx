 ...
 */
export default function TrialGateway({
  authFetch,
  agent,
  onNavigate,
  onCheckout,
  ...
  return (
    <div
      ...
      aria-modal="true"
      aria-label={
        isLocked ? 'Workspace access locked' : 'Trial ended — grace period'
      }
      style={{
        position: 'fixed',
        inset: 0,
      ...
      >
        ...
        {isLocked && (
          <p style={{ marginTop: 20, fontSize: 11, color: '#94a3b8' }}>
            Plan:{' '}
            <strong style={{ color: '#64748b' }}>{plan || 'Trial'}</strong>
            &nbsp;·&nbsp;Contact{' '}
            <a
              href="mailto:support@atelier-omnicore.com"
              style={{ color: '#7c3aed' }}
            >
              support
            </a>{' '}
