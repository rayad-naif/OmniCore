...
function StatChip({ icon, label, value, color = 'slate' }) {
  ...
  return (
    <div
      className={`flex items-center gap-2 px-3 py-2 rounded-xl ${colors[color]}`}
    >
      <span className="text-base">{icon}</span>
      <div>
        <p className="text-[10px] font-medium opacity-70 leading-none">
          {label}
        </p>
    ...
    try {
      ...
      if (contentType.includes('text/event-stream')) {
        ...
        if (data.jobId) {
          ...
          appendLog(
            'success',
            `Crawl complete: ${data.stats.crawled} pages, ${data.stats.vectorised} indexed`,
          );
        }
      }
    } catch (err) {
  ...
  const handleProgressEvent = useCallback(
    (ev) => {
      if (ev.type === 'progress' || ev.crawled !== undefined) {
        setStats({
          crawled: ev.crawled || 0,
          vectorised: ev.vectorised || 0,
          errors: ev.errors || 0,
        });
        if (ev.currentUrl) {
          setCurrentUrl(ev.currentUrl);
          appendLog('info', `Crawled: ${ev.currentUrl}`);
        }
      } else if (ev.type === 'complete' || ev.done) {
        const s = ev.stats || ev;
        setFinalStats({
          crawled: s.crawled || 0,
          vectorised: s.vectorised || 0,
          errors: s.errors || 0,
        });
        setPhase('done');
        appendLog(
          'success',
          `Finished: ${s.crawled} pages crawled, ${s.vectorised} vectorised`,
        );
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500">
                Extract and index content from any URL
              </p>
            </div>
          </div>
          <button
            onClick={phase === 'crawling' ? cancelCrawl : onClose}
            ...
            <div className="space-y-5">
              ...
              <div>
                ...
                <p className="mt-1.5 text-xs text-slate-400">
                  The crawler will follow internal links starting from this
                  page.
                </p>
              </div>
              ...
              <div className="flex items-start gap-3 px-4 py-3 rounded-xl bg-blue-50 border border-blue-200 text-blue-800 text-xs">
                ...
                <p>
                  Pages are chunked into ~500-word segments and embedded into
                  your brand's vector index. Only same-origin URLs are followed.
                  Respects{' '}
            ...
            <div className="space-y-5">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-400">
                    Scanning pages and building vector embeddings
                  </p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-3 gap-2">
                ...
                <StatChip
                  ...
                  label="Errors"
                  value={stats.errors}
                  color={stats.errors > 0 ? 'red' : 'slate'}
                />
              </div>
            ...
            <div className="space-y-5">
              ...
              <div className="grid grid-cols-3 gap-2">
                <StatChip
                  icon="📄"
                  label="Pages crawled"
                  value={finalStats.crawled ?? 0}
                  color="violet"
                />
                <StatChip
                  icon="🧠"
                  label="Indexed"
                  value={finalStats.vectorised ?? 0}
                  color="emerald"
                />
                <StatChip
                  icon="⚠️"
                  ...
                  color={finalStats.errors > 0 ? 'red' : 'slate'}
                />
              </div>
              ...
              {finalStats.errors > 0 && (
                <div className="px-4 py-3 rounded-xl bg-amber-50 border border-amber-200 text-amber-800 text-xs">
                  ...
                  <p>
                    This is normal — pages may have been blocked by robots.txt,
                    returned non-HTML content, or timed out.
                  </p>
                </div>
              )}
                ...
                <details className="group">
                  ...
                  <div className="mt-2 bg-slate-900 rounded-xl h-32 overflow-y-auto px-4 py-3 font-mono text-[11px] space-y-1">
                    {log.map((e, i) => (
                      <div
                        key={i}
                        className={`${e.type === 'error' ? 'text-red-400' : e.type === 'success' ? 'text-emerald-400' : 'text-slate-300'}`}
                      >
                ...
                <div className="bg-slate-900 rounded-xl h-32 overflow-y-auto px-4 py-3 font-mono text-[11px] space-y-1">
                  {log.slice(-20).map((e, i) => (
                    <div
                      key={i}
                      className={
                        e.type === 'error' ? 'text-red-400' : 'text-slate-300'
                      }
                    >
                      {e.ts} {e.text}
                    </div>
