...
function Spinner({ size = 'sm' }) {
  return (
    <svg
      className={`animate-spin ${size === 'sm' ? 'w-3.5 h-3.5' : 'w-4 h-4'} text-violet-600`}
      viewBox="0 0 24 24"
      fill="none"
    >
      <circle
...
function EditorToolbar({ onInsertLink }) {
  return (
    <div className="flex flex-wrap items-center gap-0.5 px-3 py-2 border-b border-slate-200 bg-slate-50">
      {/* Text style */}
      <ToolBtn command="bold" title="Bold (⌘B)">
        {' '}
        <b className="text-xs">B</b>{' '}
      </ToolBtn>
      <ToolBtn command="italic" title="Italic (⌘I)">
        {' '}
        <i className="text-xs">I</i>{' '}
      </ToolBtn>
      <ToolBtn command="underline" title="Underline (⌘U)">
        {' '}
...
export default function ArticleEditor({
  ...
  function collectAndSave(manual = true) {
    if (!title.trim()) {
      if (manual) alert('Title is required');
      return;
    }
  ...
  async function handleVectorise() {
    if (!article?.id) {
      alert('Save the article first before indexing.');
      return;
    }
    setVectorising(true);
    try {
      const res = await authFetch(
        `${API_URL}/knowledge-articles/${article.id}/vectorise`,
        {
          method: 'POST',
        },
      );
  ...
  return (
    <div className="flex flex-col h-full bg-white" onKeyDown={handleKeyDown}>
      ...
      {activeTab === 'write' ? (
        <div
          className="flex flex-col flex-1 overflow-hidden mx-6 mb-6 mt-3
        ...
        >
          ...
          <div className="px-6 py-2 border-t border-slate-100 bg-slate-50 flex justify-between text-[11px] text-slate-400">
            <span>
              {editorRef.current?.innerText?.trim().split(/\s+/).filter(Boolean)
                .length ?? 0}{' '}
              words
            </span>
            <span>⌘S to save</span>
