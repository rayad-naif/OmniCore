...
async function sendStatusChangeEmail(
  ...
  const html = brandedEmail({
    ...
    footer:
      'You received this because you opened a support ticket. Do not reply to this email.',
  });
  ...
  try {
    await buildTransporter(cfg).sendMail({
      ...
      to: visitorEmail,
      subject: `Your support ticket: ${toLabel}`,
      html,
    });
    logger.info(
      { tenantId, conversationId, newStatus, visitorEmail },
      'status_email_sent',
    );
  } catch (err) {
...
async function sendAgentReplyEmail(
  ...
  const inlineImgHtml = inlineImages
    .map(
      (im) =>
    ...
    logger.info(
      {
        ...
        agentName,
        visitorEmail,
        attachmentCount: mailerAttachments.length,
      },
      'agent_reply_email_sent',
    );
  } catch (err) {
 ...
 */
async function sendNewVisitorMessageEmail(
  tenantId,
  conversationId,
  visitorName,
  messageBody,
  ...
  const html = brandedEmail({
    ...
    footer:
      'You received this because you configured inbox notifications in OmniCore.',
  });
  ...
  try {
    ...
    logger.info(
      { tenantId, conversationId, visitorName },
      'visitor_message_notification_sent',
    );
  } catch (err) {
    logger.warn(
      { err, tenantId, conversationId },
      'visitor_message_notification_failed',
    );
  }
}
...
async function sendTicketCreatedEmail(
  ...
  try {
    await buildTransporter(cfg).sendMail(mailOpts);
    logger.info(
      { tenantId, conversationId, ticketNumber, visitorEmail },
      'ticket_created_email_sent',
    );
  } catch (err) {
    logger.warn(
      { err, tenantId, conversationId },
      'ticket_created_email_failed',
    );
  }
}
...
async function sendSmtpTestEmail(tenantId) {
  ...
  if (!cfg)
    throw Object.assign(new Error('SMTP not configured or not enabled'), {
      status: 400,
    });
...
async function sendPlatformSmtpTestEmail(toOverride) {
  ...
  if (!cfg)
    throw Object.assign(
      new Error('Platform SMTP not configured or not enabled'),
      { status: 400 },
    );
 ...
 */
async function sendAgentInviteEmail({
  to,
  name,
  inviteLink,
  companyName,
  ...
  return sendSystemEmail({
    ...
    bodyHtml,
    footer:
      'You received this because an administrator invited you to OmniCore.',
  });
}
...
async function sendPasswordResetEmail(
  ...
  const footer =
    'You received this because a password reset was requested for your OmniCore account.';

  // 1. Try platform SMTP first.
  const sentViaPlatform = await sendSystemEmail({
    to: agentEmail,
    subject: 'OmniCore: Reset your password',
