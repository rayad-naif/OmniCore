...
const PRIVATE_HOST_RE =
  /^(localhost|127\.|10\.|172\.(1[6-9]|2[0-9]|3[01])\.|192\.168\.|169\.254\.|::1$|fc00:|fe80:|0\.0\.0\.0$)/i;
...
router.get('/knowledge-base', async (req, res, next) => {
  try {
    ...
    if (brand_id) {
      where += ` AND (brand_id = $${params.length + 1} OR brand_id IS NULL)`;
...
router.post('/knowledge-base', async (req, res, next) => {
  try {
    ...
    logger.info(
      { id: rows[0].id, agentId: req.agent.id },
...
router.patch(
  '/settings/:brandId',
  requirePermission('knowledge_base', 'edit'),
  async (req, res, next) => {
    try {
      const {
        ai_system_prompt,
      ...
      const botPatch = {};
      if (bot_max_messages !== undefined)
        botPatch.bot_max_messages =
          parseInt(String(bot_max_messages), 10) || 10;
      if (auto_assign_strategy !== undefined)
        botPatch.auto_assign_strategy = auto_assign_strategy;
      if (auto_close_enabled !== undefined)
        botPatch.auto_close_enabled = Boolean(auto_close_enabled);
      if (auto_close_idle_minutes !== undefined)
        botPatch.auto_close_idle_minutes =
          parseInt(String(auto_close_idle_minutes), 10) || 60;
  ...
  try {
    ...
    try {
      ...
      if (PRIVATE_HOST_RE.test(parsed.hostname))
        return res
          .status(400)
          .json({ error: 'Private/local URLs are not permitted' });
    ...
    const maxPages = Math.min(
      Math.max(parseInt(String(max_pages)) || 100, 1),
      5000,
    );
    const maxDepth = Math.min(
      Math.max(parseInt(String(max_depth)) || 5, 1),
      20,
    );
...
router.post('/knowledge-base/upload-pdf', async (req, res, next) => {
  try {
    ...
    if (!data)
      return res.status(400).json({ error: 'data (base64 PDF) is required' });
