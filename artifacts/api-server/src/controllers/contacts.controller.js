...
router.get('/export', async (req, res, next) => {
  try {
    ...
    const HEADER =
      'Name,Email,Brand,Location,Last Seen,First Seen,Conversations\r\n';
    res.write(HEADER);
...
router.get('/', async (req, res, next) => {
  try {
    ...
    const [{ rows: contacts }, { rows: countRow }] = await Promise.all([
      ...
      pool.query(`SELECT COUNT(*) FROM visitors v WHERE ${where}`, values),
    ]);
...
router.delete('/:visitorId', async (req, res, next) => {
  try {
    ...
    logger.info(
      { visitorId: req.params.visitorId, by: req.agent.id },
      'contact_deleted',
    );
