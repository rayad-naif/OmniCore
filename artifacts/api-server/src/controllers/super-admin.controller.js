...
router.patch('/tenants/:id/status', async (req, res, next) => {
  try {
    ...
    logger.info(
      { tenantId: req.params.id, account_status, by: req.agent.id },
  ...
  try {
    ...
    if (!fields.length)
      return res.status(400).json({
        error:
          'Provide at least one of: plan, subscription_status, trial_ends_at, grace_period_ends_at, lock_notified_at',
      });
    ...
    if (!rows.length)
      return res.status(404).json({ error: 'Tenant not found' });
    logger.info(
      { tenantId: req.params.id, fields, by: req.agent.id },
...
router.post('/tenants/:id/checkout', async (req, res, next) => {
  try {
    ...
    logger.info(
      { tenantId: req.params.id, plan, by: req.agent.id },
      'superadmin_checkout_generated',
    );
  ...
  try {
    ...
    if (max_brands_allowed !== undefined) {
      ...
      if (isNaN(val) || val < 1 || val > 1000)
        return res
          .status(400)
          .json({ error: 'max_brands_allowed must be 1–1000' });
      updates.push(`max_brands_allowed = $${i++}`);
      values.push(val);
    }
    if (max_agents_allowed !== undefined) {
      ...
      if (isNaN(val) || val < 1 || val > 10000)
        return res
          .status(400)
          .json({ error: 'max_agents_allowed must be 1–10000' });
      updates.push(`max_agents_allowed = $${i++}`);
      values.push(val);
    }
    if (ai_feature_enabled !== undefined) {
      updates.push(`ai_feature_enabled = $${i++}`);
      values.push(Boolean(ai_feature_enabled));
    }
    if (smtp_feature_enabled !== undefined) {
      updates.push(`smtp_feature_enabled = $${i++}`);
      values.push(Boolean(smtp_feature_enabled));
    }
    if (conversation_limit !== undefined) {
      ...
      if (isNaN(val) || val < 1)
        return res
          .status(400)
          .json({ error: 'conversation_limit must be >= 1' });
      updates.push(`conversation_limit = $${i++}`);
      values.push(val);
    }

    if (!updates.length)
      return res.status(400).json({ error: 'No valid limit fields provided' });
    ...
    if (!rows.length)
      return res.status(404).json({ error: 'Tenant not found' });
    logger.info(
      { tenantId: req.params.id, by: req.agent.id },
...
router.post('/super-admins', async (req, res, next) => {
  try {
    ...
    logger.info(
      { email: normalized, by: req.agent.email },
      'super_admin_added',
    );
...
router.delete('/super-admins/:id', async (req, res, next) => {
  try {
    ...
    logger.info(
      { id: req.params.id, by: req.agent.email },
      'super_admin_removed',
    );
...
router.patch('/users/:id/password', async (req, res, next) => {
  try {
    ...
    logger.info(
      { targetId: req.params.id, by: req.agent.email },
      'user_password_set_by_super_admin',
    );
    ...
    logger.info(
      {
        ...
        by: req.agent.email,
        sent,
        variant: isInvite ? 'invite' : 'workspace',
      },
      'user_reset_email_sent_by_super_admin',
    );
    return res.json({
      ok: true,
      sent,
...
router.put('/platform-smtp', async (req, res, next) => {
  try {
    ...
    const cfg = {
      ...
      pass:
        body.pass && String(body.pass).trim().length
          ? String(body.pass)
          : existing.pass || '',
    };
...
router.post('/platform-smtp/test', async (req, res) => {
  try {
    ...
    logger.warn(
      { err: err.message, by: req.agent.email },
      'platform_smtp_test_failed',
    );
...
router.delete('/tenants/:id/purge', async (req, res, next) => {
  ...
  try {
    ...
    if (confirm_name !== rows[0].company_name) {
      await client.query('ROLLBACK');
      return res.status(400).json({
        error: 'Confirmation name does not match tenant company_name',
      });
    }
...
router.get('/billing/status', async (req, res, next) => {
  try {
    ...
    try {
      ...
      /* tenants table always exists; ignore transient errors */
    }
...
router.post('/billing/plans', async (req, res, next) => {
  try {
    ...
    return res
      .status(500)
      .json({ error: err.message || 'Failed to create plan' });
  }
});
...
router.patch('/billing/plans/:id', async (req, res, next) => {
  try {
    const { plan, warning } = await plansRepo.updatePlan(
      req.params.id,
      req.body || {},
    );
    ...
    return res
      .status(500)
      .json({ error: err.message || 'Failed to update plan' });
  }
});
...
router.delete('/billing/plans/:id', async (req, res, next) => {
  try {
    ...
    return res
      .status(500)
      .json({ error: err.message || 'Failed to delete plan' });
  }
});
