...
TENANT_MIGRATIONS.forEach((sql) =>
  pool
    .query(sql)
    .catch((err) =>
      logger.warn({ err: err.message }, 'tenant_migration_warning'),
    ),
);
...
router.patch('/settings', requireRole('admin'), async (req, res, next) => {
  try {
    ...
    if (!rows.length) return notFound(res, 'Tenant');
    logger.info(
      { tenantId: req.agent.tenantId, fields },
...
router.post('/smtp/test', requireRole('admin'), async (req, res, next) => {
  try {
    ...
    logger.warn(
      { err: err.message, tenantId: req.agent.tenantId },
      'smtp_test_failed',
    );
...
function getR2Client() {
  ...
  if (!accountId)
    throw Object.assign(new Error('R2_ACCOUNT_ID not set'), { status: 503 });
  return new S3Client({
    region: 'auto',
  ...
  async (req, res, next) => {
    try {
      ...
      if (filename && !ALLOWED_LOGO_EXT.test(filename)) {
        return res
          .status(400)
          .json({ error: 'Filename must be .png, .jpg, .jpeg, or .webp' });
      }
      if (
        size !== undefined &&
        (typeof size !== 'number' || size > MAX_LOGO_BYTES)
      ) {
        return res.status(400).json({
          error: `File must not exceed ${MAX_LOGO_BYTES / 1024 / 1024} MB`,
        });
      }
      ...
      if (!bucket)
        return res.status(503).json({ error: 'R2_BUCKET_NAME not set' });
